...
def getNewDeckCountsAfterRemoveCard(deckCounts: Tuple, indexToRemove: int):
    deckCountsList = list(deckCounts)
    deckCountsList[indexToRemove] -= 1

    if sum(deckCountsList) == 0:
        return None

    return tuple(deckCountsList)


class BlackjackMDP(util.MDP):
    def __init__(
        self, cardValues: List[int], multiplicity: int, threshold: int, peekCost: int
    ):
        """
    ...
    def getNewStateAndRewardFromStateAndIndex(self, state: State, index: int) -> Tuple:
        # Create new State
        newHandTotal = state.handTotal + self.cardValues[index]
        isHandBusted = newHandTotal > self.threshold
        newDeckCounts = (
            None
            if isHandBusted
            else getNewDeckCountsAfterRemoveCard(state.deckCounts, index)
        )
        ...
        # Calculate reward
        if newDeckCounts != None or isHandBusted:
            reward = 0
        else:
            # deck runs out of card
            reward = newHandTotal

        return (newState, reward)

    def getPossibleResultsFromTakeAction(self, state: State) -> List[PossibleResult]:
        # Card was peaked
        if state.nextCard != None:
            newState, reward = self.getNewStateAndRewardFromStateAndIndex(
        ...
        res = []
        curDeckCounts = state.deckCounts
        totalDeckCount = sum(curDeckCounts)
        # iterate through the deck to generate new possible results
        for i in range(len(curDeckCounts)):
            if curDeckCounts[i] == 0:
                continue

            newState, reward = self.getNewStateAndRewardFromStateAndIndex(state, i)
            res.append(
                PossibleResult(
                    successor=newState,
                    probability=curDeckCounts[i] / totalDeckCount,
                    reward=reward,
                )
            )
        return res

    def getPossibleResultsFromPeekAction(self, state: State) -> List[PossibleResult]:
        # Check if peak twice in a row
        if state.nextCard != None:
            return []
    ...
    def succAndProbReward(self, state: State, action: str) -> List[PossibleResult]:
        ...
        if action == "Quit":
            newState = State(
                handTotal=state.handTotal, nextCard=None, deckCounts=None
            )  # specified in assignment
            return [
                PossibleResult(
                    successor=newState, probability=1, reward=state.handTotal
                )
            ]

        elif action == "Take":
            return self.getPossibleResultsFromTakeAction(state)

        elif action == "Peek":
            return self.getPossibleResultsFromPeekAction(state)
...
class QLearningAlgorithm(util.RLAlgorithm):
    ...
    def incorporateFeedback(
        self, state: State, action: Any, reward: int, newState: State
    ) -> None:
        qPrime = (
            max(self.getQ(newState, action) for action in self.actions(state))
            if newState != None
            else 0
        )

        featureValues = self.featureExtractor(state, action)
        difference = (reward + self.discount * qPrime) - self.getQ(state, action)
        for featureKey, featureValue in featureValues:
            self.weights[featureKey] += self.getStepSize() * difference * featureValue
...
def simulate_QL_over_MDP(mdp: BlackjackMDP, featureExtractor: Callable):
    ...
    for state in mdp.states:
        ...
        totalError += math.fabs(
            alg.V[state] - qlearn.getQ(state, qlearn.getAction(state))
        )
    ...
    print(
        "Number of states where Q-learning action matches value iteration action:",
        correctCount,
    )
    ...
    print()
    print(
        "Avg rewards from value iteration result = ",
...
def blackjackFeatureExtractor(state: State, action: str) -> List[Feature]:
    ...
    # indicator for the action and the current total
    features.append(
        Feature(featureKey=("total", state.handTotal, action), featureValue=1)
    )

    if state.deckCounts != None:
        # indicator for the action and presence/absence of each face value in the deck
        deckCounts = tuple([1 if count > 0 else 0 for count in state.deckCounts])
        features.append(
            Feature(featureKey=("bitmask", deckCounts, action), featureValue=1),
        )

        # indicator for the action and the number of cards remaining with each face value
        for i in range(len(state.deckCounts)):
            features.append(
                Feature(featureKey=(i, state.deckCounts[i], action), featureValue=1)
            )
