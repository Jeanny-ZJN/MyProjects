...
class MinimaxPlayer(Player):
    ...
    def choose_move(self, board: GameBoard) -> Optional[Location]:
        ...
        if active_player == MAX_PLAYER:
            _, best_location = self.max_values(board, 0)
            return best_location
        else:
            _, best_location = self.min_values(board, 0)
            return best_location

    def max_values(self, board: GameBoard, current_plies: int):
        ...
        for possible_move in board.get_legal_moves():
            ...
            # mutual recursion
            local_best_h, _ = self.min_values(board_copy, current_plies + 1)
    ...
    def min_values(self, board: GameBoard, current_plies: int):
        if current_plies == self.plies:
            # will know the location from where this function had been
            # recursively called
            return self.heuristic(board), None
        ...
        for possible_move in board.get_legal_moves():
            board_copy = board.make_move(possible_move)
            # mutual recursion
            local_best_h, _ = self.max_values(board_copy, current_plies + 1)
